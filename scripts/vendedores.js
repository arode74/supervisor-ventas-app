function notifyMsg(msg){ try{ if(typeof mostrarAlerta==="function") return mostrarAlerta(msg);}catch(_){} try{window.alert(msg);}catch(_){} }
...
async function guardarEdicionVendedor(e) {
  ...
  if (!vendedorEditando) {
    notifyMsg("No hay un vendedor seleccionado para editar.");
    return;
  }
  ...
  if (!nombreEdit || !fechaIngEdit) {
    notifyMsg("Debes completar el nombre y la fecha de ingreso.");
    return;
  }
  ...
  if (revertirBaja) {
    ...
    try {
      ...
      const { data: dataVend, error: errVend } = await supabase
        .from("vendedores")
        .update({
          nombre:        nombreNormalizado,
          fecha_ingreso: fechaIngEdit,
      ...
      if (!dataVend || (Array.isArray(dataVend) && dataVend.length === 0)) {
...
if (errVend) {
      ...
      const { data: dataEV, error: errEV } = await supabase
        .from("equipo_vendedor")
        .update({
          fecha_fin: null,
          estado:    true,
...
btnGuardarEdicion?.addEventListener("click", guardarEdicionVendedor);

// Compatibilidad: si el HTML del modal editar usa el botón id=btnGuardarVendedor (method=dialog), capturamos el click y ejecutamos el guardado.
document.addEventListener(
  "click",
  async (ev) => {
    const btn = ev.target?.closest?.('#modalEditarVendedor #btnGuardarVendedor, #modalEditarVendedor #btnGuardarEdicion');
    if (!btn) return;
    // Evita cierre automático del <dialog> por submit/method=dialog
    ev.preventDefault();
    ev.stopPropagation();
    if (typeof ev.stopImmediatePropagation === 'function') ev.stopImmediatePropagation();
    await guardarEdicionVendedor(ev);
  },
  true
);
